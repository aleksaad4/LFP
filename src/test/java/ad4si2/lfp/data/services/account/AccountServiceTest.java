...
public class AccountServiceTest extends BaseTest {
    ...
    public void testValidation() {
        final Account account = accountService.create(new Admin("login", "password", AccountRole.ADMIN));

        // нельзя создать с таким же логином
        Assert.assertTrue(accountService.validateEntry(new Admin("login", "password", AccountRole.ADMIN), false).hasErrors());
        // подаём на редактирование другую запись с таким же логином
        Assert.assertTrue(accountService.validateEntry(new Admin("login", "password", AccountRole.ADMIN), true).hasErrors());
